...
function powMap(power, start, end, method = "replace", thisValue) {
    if (!power) {
        throw new Error("Power is not defined");
    }
...
function multiplyMap(multiplier, start, end, method = "replace", thisValue) {
    if (!multiplier) {
        throw new Error("Multiplier is not defined");
    }
...
function randomRange(count, multiplier) {
    if (!count) {
        throw new Error("Count [minimal range number] is not defined");
    }
...
function index(item, start, end, thisValue) {
    ...
    if (!!start && !!end) {
        return [...this.splice(start, (end < this.length) ? end - start + 1 : this.length)].map((i, idx) => { return { "item": i, "index": idx }; }).filter((i) => { return i.item === item; });
    }
    if (!!start && !end) {
        return [...this.splice(0, (start < this.length) ? start : this.length)].map((i, idx) => { return { "item": i, "index": idx }; }).filter((i) => { return i.item === item; });
    }
...
function diction(mapperFunction, arg, start, end, keyconvertor = "index", thisValue, keyArrayValue) {
    ...
    if (!mapperFunction || typeof mapperFunction !== "function") {
        mapperFunction = (i) => i;
    }
    if (keyconvertor === "value") {
        aO = a.reduce((a, c, i) => { return Object.assign(Object.assign({}, a), { [c]: c }); }, aO);
    }
    else if (keyconvertor === "index") {
        aO = Object.assign(aO, a);
    }
    else if (keyconvertor === "array") {
        if (a.length < keyArrayValue.length)
            throw new Error("[ERROR:] keyArrayValue array length less than array length");
        aO = a.reduce((a, c, i) => { return Object.assign(Object.assign({}, a), { [keyArrayValue[i]]: c }); }, aO);
    }
    if (!!mapperFunction && typeof mapperFunction === "function")
        return mapperFunction(aO, arg);
    return aO;
}
/**
 *
...
function range(start, stop, step) {
    if (((!start && !stop) || !start) && (start !== 0) && (stop !== 0)) {
        throw new Error("Start [minimal range end number] is not defined");
    }
    let a = [];
